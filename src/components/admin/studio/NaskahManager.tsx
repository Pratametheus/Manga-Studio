...
const MenuBar = ({ editor }: { editor: any }) => {
  ...
  const btnClass = "p-2 rounded-md hover:bg-gray-100 dark:bg-slate-900 transition-colors text-gray-700 dark:text-slate-300";
  const activeClass = "bg-indigo-100 text-indigo-700";

  return (
    <div className="flex flex-wrap items-center gap-1 p-2 bg-gray-50 dark:bg-[#111] border-b border-gray-200 dark:border-white/10 rounded-t-xl">
...
export function NaskahManager({ mangaId }: NaskahManagerProps) {
  ...
  return (
    <div className="flex gap-6 h-[calc(100vh-14rem)]">
      ...
      <div className="w-64 shrink-0 flex flex-col bg-white dark:bg-[#0a0a0a] rounded-2xl border border-gray-200 dark:border-white/10 overflow-hidden shadow-sm">
        <div className="p-4 border-b border-gray-100 dark:border-white/5 flex justify-between items-center bg-gray-50 dark:bg-[#111]/50 dark:bg-slate-950">
          <h3 className="font-bold text-gray-900 dark:text-white">Daftar Bab</h3>
          <button 
          ...
          {naskahs.map(n => (
            <div 
              ...
              className={cn(
                ...
                selectedNaskah?.id === n.id ? "bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-100 shadow-sm" : "hover:bg-gray-50 dark:bg-[#111] border border-transparent"
              )}
            >
              <div className="overflow-hidden pr-2">
                <p className={cn("text-sm font-bold truncate", selectedNaskah?.id === n.id ? "text-indigo-900" : "text-gray-700 dark:text-slate-300")}>
                  Bab {n.bab}
                </p>
                <p className={cn("text-xs truncate", selectedNaskah?.id === n.id ? "text-indigo-600 dark:text-indigo-400" : "text-gray-500 dark:text-slate-400")}>
      ...
      <div className="flex-1 bg-white dark:bg-[#0a0a0a] rounded-2xl border border-gray-200 dark:border-white/10 shadow-sm flex flex-col overflow-hidden">
        {selectedNaskah ? (
          <>
            <div className="p-4 border-b border-gray-100 dark:border-white/5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-gray-50 dark:bg-[#111]/50 dark:bg-slate-950">
              <div className="flex-1 flex items-center gap-3">
                <span className="font-bold text-gray-900 dark:text-white whitespace-nowrap">Bab {selectedNaskah.bab} :</span>
                <input 
                  type="text"
                  value={judulBab}
                  onChange={(e) => setJudulBab(e.target.value)}
                  placeholder="Judul Bab (Opsional)"
                  className="flex-1 bg-white dark:bg-[#0a0a0a] border border-gray-200 dark:border-white/10 px-3 py-1.5 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 font-medium"
                />
              </div>
              <div className="flex items-center gap-3">
                <select 
                  ...
                  className="bg-white dark:bg-[#0a0a0a] border border-gray-200 dark:border-white/10 px-3 py-1.5 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 text-gray-700 dark:text-slate-300"
                >
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-gray-400 dark:text-slate-500">
            ...
            <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">Editor Naskah Cerita</h3>
        ...
        <form onSubmit={handleSubmit(handleAddNaskah)} className="space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:border-white/10 focus:ring-2 focus:ring-indigo-500 outline-none"
              placeholder="Contoh: 1"
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">Judul Bab (Opsional)</label>
            <input
              {...register('judul_bab')}
              className="w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:border-white/10 focus:ring-2 focus:ring-indigo-500 outline-none"
