...
export function LandingTeamTab() {
  ...
  if (loading) return <div className="p-6 text-center text-gray-500 dark:text-slate-400">Memuat Tim...</div>;

  return (
    <div className="p-6 space-y-6">
      <div className="flex justify-between items-center border-b pb-4">
        <div>
          <h2 className="text-lg font-bold text-gray-900 dark:text-white">Manajemen Tim Kreator</h2>
          <p className="text-sm text-gray-500 dark:text-slate-400">Atur anggota tim yang tampil di Dapur Kreatif pada Landing Page.</p>
        </div>
        {!isAdding && !editingId && (
          <button 
            onClick={() => {
        ...
        <form onSubmit={isAdding ? handleAdd : (e) => { e.preventDefault(); handleUpdate(editingId!); }} className="bg-gray-50 dark:bg-[#111] p-5 rounded-xl border border-gray-200 dark:border-white/10 space-y-4">
          <div className="flex flex-col sm:flex-row gap-6">
            <div className="shrink-0">
              <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">Foto Profil</label>
              <div 
                onClick={() => fileInputRef.current?.click()} 
                className="w-32 h-32 rounded-full bg-gray-200 dark:bg-slate-800 border-2 border-dashed border-gray-300 dark:border-white/20 flex items-center justify-center cursor-pointer overflow-hidden relative group hover:border-indigo-400 transition-colors"
              >
                {formData.avatar_url ? (
            ...
            <div className="flex-1 space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">Nama Pena / Nama Asli *</label>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">Role (Peran) *</label>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">Bio Singkat (Opsional)</label>
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-gray-200 dark:border-white/10 mt-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {creators.map(creator => (
          <div key={creator.id} className="p-4 border border-gray-200 dark:border-white/10 rounded-xl hover:border-indigo-300 transition-colors bg-white dark:bg-[#0a0a0a] flex flex-col">
            <div className="flex items-start gap-4 mb-4">
              <div className="w-16 h-16 rounded-full bg-gray-100 dark:bg-slate-900 overflow-hidden shrink-0 border border-gray-200 dark:border-white/10">
                {creator.avatar_url ? (
                  <img src={creator.avatar_url} alt={creator.nama_pena} className="w-full h-full object-cover" />
                ) : (
                  <div className="w-full h-full flex items-center justify-center text-gray-400 dark:text-slate-500 font-bold text-xl">{creator.nama_pena.charAt(0)}</div>
                )}
              </div>
              <div className="flex-1 min-w-0">
                <h4 className="font-bold text-gray-900 dark:text-white truncate">{creator.nama_pena}</h4>
                <p className="text-indigo-600 dark:text-indigo-400 text-xs font-bold uppercase tracking-widest mt-0.5 truncate">{creator.role}</p>
                <span className="inline-block mt-2 px-2 py-0.5 bg-gray-100 dark:bg-slate-900 text-gray-600 dark:text-slate-400 text-[10px] font-bold rounded">Urutan: {creator.order_index}</span>
              </div>
            </div>
            {creator.bio && <p className="text-gray-600 dark:text-slate-400 text-sm line-clamp-2 mb-4 flex-1">{creator.bio}</p>}
            <div className="flex items-center gap-2 mt-auto pt-3 border-t border-gray-100 dark:border-white/5">
              <button onClick={() => startEdit(creator)} className="flex-1 flex items-center justify-center gap-2 py-1.5 text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-500/10 hover:bg-indigo-100 rounded-lg transition-colors text-sm font-medium">
        ...
        {creators.length === 0 && !isAdding && (
          <div className="col-span-full text-center p-8 border-2 border-dashed border-gray-200 dark:border-white/10 rounded-xl text-gray-500 dark:text-slate-400">
